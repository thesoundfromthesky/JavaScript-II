...
// and has invited 50 small businesses to make a small donation on
...
let fullNames = [];
runners.forEach(({ first_name: fn, last_name: ln }) => {
  fullNames.push(`${fn} ${ln}`);
});
console.log(fullNames);

// ==== Challenge 2: Use .map() ====
// The event director needs to have all the runners' first names in uppercase
// because the director BECAME DRUNK WITH POWER. Populate an array called `firstNamesAllCaps`.
// This array will contain just strings.
let firstNamesAllCaps = [];
runners.forEach(({ first_name: fn, last_name: ln }) => {
  firstNamesAllCaps.push(`${fn.toUpperCase()} ${ln}`);
});
...
runners
  .filter(({ shirt_size: ss }) => ss === "L")
  .forEach(({ first_name: fn, last_name: ln }) => {
    runnersLargeSizeShirt.push(`${fn} ${ln}`);
  });
...
let descendingOrder = [];
descendingOrder = [...runners].sort(
  ({ donation: a }, { donation: b }) => b - a
);
...
// map company name
let company = [];
company = runners.map(({ company_name: ca }) => ca);
console.log(company);

// Problem 3
let beyond = [];
beyond = runners.map(({ shirt_size: ss }) => ss.replace(/3XL/i, "Beyond"));
console.log(beyond);